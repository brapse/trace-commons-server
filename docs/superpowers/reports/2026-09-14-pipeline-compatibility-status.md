...
```bash
cargo test -p trace-commons-server compatibility_baseline_is_executable_and_detects_drift
```

The test loads the baseline and corpus from `docs/superpowers/specs/`.
The test fails after a decision, floor, corpus order, or initial index change.
...
The local bundle uses the reference scorer with zero gate floors.
The bundle configuration marks this policy as non-qualifiable.

A production-compatible bundle must use nonzero quality and novelty floors.
Package construction rejects a production-compatible bundle with a zero floor.
